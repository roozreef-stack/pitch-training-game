...
class ScaleManager {
    ...
    getRandomScale(type = 'all') {
        let availableScales;

        if (type === 'major') {
            availableScales = this.majorScales;
        } else if (type === 'minor') {
            availableScales = this.minorScales;
        } else {
            availableScales = this.allScales;
        }

        const randomIndex = Math.floor(Math.random() * availableScales.length);
        return availableScales[randomIndex];
    }
...
class PitchTrainingGame {
    ...
    async startNewQuestion() {
        ...
        // ランダムなスケールを選択
        this.currentScale = this.scaleManager.getRandomScale(scaleType);
